...
int main(void)
{
    ...
    if (rslt == BMI2_OK)
    {
        ...
        rslt = bmi270_legacy_sensor_enable(sensor_sel, 2, &bmi2_dev);
        bmi2_error_codes_print_result(rslt);
        ...
        if (rslt == BMI2_OK)
        {
            ...
            if (rslt == BMI2_OK)
            {
                ...
                rslt = bmi270_legacy_map_feat_int(&sens_int, 1, &bmi2_dev);
                bmi2_error_codes_print_result(rslt);
                ...
                while (loop)
                {
                    ...
                    if (int_status & BMI270_LEGACY_INT_STEP_ACTIVITY_MASK)
                    {
                        printf("Step detected\n");

                        /* Get step activity output. */
                        rslt = bmi270_legacy_get_feature_data(&sensor_data, 1, &bmi2_dev);
                        bmi2_error_codes_print_result(rslt);
