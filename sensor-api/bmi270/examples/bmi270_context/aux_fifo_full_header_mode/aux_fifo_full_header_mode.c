...
int main(void)
{
    ...
    /* As per datasheet, aux interface with bmi270_context will support only for I2C */
    ...
    rslt = bmi270_context_init(&aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    rslt = bmi270_context_get_sensor_config(config, 3, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    rslt = bmi270_context_set_sensor_config(config, 3, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);

    /* NOTE:
     * Accel and gyro enable must be done after setting configurations
     */
    rslt = bmi270_context_sensor_enable(sensor_list, 3, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
