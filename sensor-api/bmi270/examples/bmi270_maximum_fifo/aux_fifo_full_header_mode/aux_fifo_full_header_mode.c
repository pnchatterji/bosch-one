...
#define BMI2_FIFO_RAW_DATA_BUFFER_SIZE  UINT16_C(6144)

/*! Length of data to be read from FIFO. */
#define BMI2_FIFO_RAW_DATA_USER_LENGTH  UINT16_C(6144)

/*! Number of accel frames to be extracted from FIFO. */

/*! Calculation for frame count: Total frame count = Fifo buffer size(6144)/ Total frames(6 Accel, 6 Gyro, 8 Aux and 1 header,
 * totaling to 21) which equals to 292.
 *
 * Added extra frames to parse sensortime
 */

/*! Number of accel frames to be extracted from FIFO */
#define BMI2_FIFO_ACCEL_FRAME_COUNT     UINT16_C(450)

/*! Number of gyro frames to be extracted from FIFO. */
#define BMI2_FIFO_GYRO_FRAME_COUNT      UINT16_C(450)

/*! Number of aux frames to be extracted from FIFO. */
#define BMI2_FIFO_AUX_FRAME_COUNT       UINT16_C(450)

/*! Macro to read sensortime byte in FIFO. */
#define SENSORTIME_OVERHEAD_BYTE        UINT16_C(800)
...
int main(void)
{
    ...
    rslt = bmi270_maximum_fifo_init(&aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    rslt = bmi2_get_sensor_config(config, 3, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    rslt = bmi2_set_sensor_config(config, 3, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);

    /* NOTE:
     * Accel and gyro enable must be done after setting configurations
     */
    rslt = bmi2_sensor_enable(sensor_list, 3, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
