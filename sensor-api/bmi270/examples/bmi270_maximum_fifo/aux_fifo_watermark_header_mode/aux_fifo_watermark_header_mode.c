...
/*!
 * Calculation:
 * fifo_watermark_level = 4050, accel_frame_len = 6, gyro_frame_len = 6,
 * aux_frame_len = 8, header_byte = 1.
 * fifo_accel_frame_count = (4050 / (6 + 6 + 8 + 1 )) = 192 frames
 * NOTE: Extra frames are read in order to get sensor time
 */
#define BMI2_FIFO_ACCEL_FRAME_COUNT     UINT16_C(400)

/*! Number of gyro frames to be extracted from FIFO */
#define BMI2_FIFO_GYRO_FRAME_COUNT      UINT16_C(400)

/*! Number of aux frames to be extracted from FIFO. */
#define BMI2_FIFO_AUX_FRAME_COUNT       UINT16_C(400)

/*! Setting a watermark level in FIFO */
#define BMI2_FIFO_WATERMARK_LEVEL       UINT16_C(4050)
