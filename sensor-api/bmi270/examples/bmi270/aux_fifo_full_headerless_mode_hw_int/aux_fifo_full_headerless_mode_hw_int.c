...
int main(void)
{
    ...
    rslt = bmi2_get_int_pin_config(&pin_config, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    rslt = bmi2_set_int_pin_config(&pin_config, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    {
        coines_attach_interrupt(COINES_SHUTTLE_PIN_20, interrupt_callback, COINES_PIN_INTERRUPT_FALLING_EDGE);
    }

#if !defined(MCU_APP20)
    else if (board == BOARD_MCU_APP30)
    {
        coines_attach_interrupt(COINES_MINI_SHUTTLE_PIN_1_6, interrupt_callback, COINES_PIN_INTERRUPT_FALLING_EDGE);
    }
#endif
                    ...
                    if (rslt == BMI2_OK)
                    {
                        ...
                        for (index = 0; index < aux_frame_length; index++)
                        {
                            ...
                            printf("AUX[%d] Mag_uT_X : %ld\t Mag_uT_Y : %ld\t Mag_uT_Z : %ld\n", index,
                                   (long unsigned int)mag_data.x, (long unsigned int)mag_data.y,
