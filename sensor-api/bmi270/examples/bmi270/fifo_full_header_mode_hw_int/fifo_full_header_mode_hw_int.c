...
int main(void)
{
    ...
    * 157 * (6 axes + 1 header bytes) = 1099 bytes */
    ...
    rslt = bmi2_map_data_int(fifoframe.data_int_map, BMI2_INT2, &bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    {
        coines_attach_interrupt(COINES_MINI_SHUTTLE_PIN_1_7, interrupt_callback, COINES_PIN_INTERRUPT_FALLING_EDGE);
    }
#endif
