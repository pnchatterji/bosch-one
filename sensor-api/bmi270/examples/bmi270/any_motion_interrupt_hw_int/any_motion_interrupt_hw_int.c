...
int main(void)
{
    ...
    if (rslt == BMI2_OK)
    {
        ...
        if (rslt == BMI2_OK)
        {
            ...
            {
                coines_attach_interrupt(COINES_SHUTTLE_PIN_21, interrupt_callback, COINES_PIN_INTERRUPT_CHANGE);
            }

#if !defined(MCU_APP20)
            else if (board == BOARD_MCU_APP30)
            {
                coines_attach_interrupt(COINES_MINI_SHUTTLE_PIN_1_7, interrupt_callback, COINES_PIN_INTERRUPT_CHANGE);
            }
#endif
