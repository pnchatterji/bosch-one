...
extern uint8_t streaming_resp_buff[COM_READ_BUFF_SIZE];

//extern uint8_t multi_io_map[COINES_SHUTTLE_PIN_MAX];
//TODO: not available in Z-COINES. It is used at only one place (See TODO comment in line1317).
...
/*! Update timestamp when interrupt is triggered */
...
static void streaming_single_fifo_send_response(stream_descriptor_t *stream_p, uint16_t read_bytes, uint8_t last_frame)
{
    ...
    while (data_available > DECODER_STRM_FIFO_RSP_PL_SZ)
    {
        ...
        terminator_pos = bytes_transmitted + DECODER_STRM_FIFO_RSP_OVERHEAD;
        tx_buf[terminator_pos] = DECODER_CR_MACRO;
        tx_buf[terminator_pos + 1] = DECODER_LF_MACRO;
        tx_buf[1] = bytes_transmitted + DECODER_STRM_FIFO_RSP_OVERHEAD + DECODER_END_INDICATORS_SIZE; /* Size field
                                                                                                       * updated*/
    ...
    if (last_frame)
    {
        ...
        terminator_pos = data_available + DECODER_STRM_FIFO_RSP_OVERHEAD;
        tx_buf[terminator_pos] = DECODER_CR_MACRO;
        tx_buf[terminator_pos + 1] = DECODER_LF_MACRO;
    ...
    else
    {
        ...
        memcpy(&residue_buffer[0], &streaming_resp_buff[bytes_transmitted],
               (uint32_t)(datalength - bytes_transmitted));
...
static int8_t read_stream_sample(stream_descriptor_t *stream_p)
{
    ...
    if ((stream_p->read_mode == STREAM_READ_N_CHUNKS) || (stream_p->read_mode == STREAM_READ_2_CHUNKS_POLLING))
    {
        ...
        if (stream_settings.ts_mode == STREAM_USE_TIMESTAMP)
        {
            if (stream_settings.stream_mode == STREAM_MODE_INTERRUPT)
            {
                stream_p->packet_timestamp_us = (timestamp_ns / 1000);
            }
            else
            {
    ...
    for (uint8_t int_idx = 0; int_idx < stream_active_count; int_idx++)
    {
        ...
        if (stream_p->data_ready_int == multi_io)
        {
            if (stream_p->intline_info != NULL)
            {
                ...
                //Same code also exists in coines_bridge_firmware.c Fix it there too
 ...
 */
void stream_interrupt_data_acq(uint32_t pins_active)
{
    stream_descriptor_t *stream_p;

    for (uint8_t i = 0; (i < stream_active_count) && (pins_active); i++)
    {
        stream_p = &stream_descriptors[i];
        if (stream_p->mode == STREAM_MODE_INTERRUPT)
        {
            ...
            if ((1 << stream_p->data_ready_int) & pins_active)
            {
                pins_active &= ~((uint32_t) (1 << stream_p->data_ready_int));
                if (stream_p->read_mode == STREAM_READ_N_CHUNKS)
                {
                    if (read_stream_sample(stream_p) == 0)
                    {
