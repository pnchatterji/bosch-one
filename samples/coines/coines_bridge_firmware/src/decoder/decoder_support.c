...

{
    ...
    enum coines_i2c_mode mode = COINES_I2C_STANDARD_MODE;
    decoder_rsp_t rsp = DECODER_RSP_SUCCESS;
    uint8_t terminating_char_pos;
    ...
    if (dir == DECODER_DIR_SET)
    {
        ...
        {
            case 0:
                mode = COINES_I2C_STANDARD_MODE;
                break;
            case 1:
                mode = COINES_I2C_FAST_MODE;
                break;
            case 2:
                mode = COINES_I2C_SPEED_3_4_MHZ;
                break;
            case 3:
                mode = COINES_I2C_SPEED_1_7_MHZ;
                break;
            default:
                break;
        ...
        if (bus_speed <= (uint8_t) COINES_I2C_SPEED_1_7_MHZ)
        {
            if (bus_index == 0)
            {
                coines_config_i2c_bus(COINES_I2C_BUS_0, mode);
            } else
                rsp = DECODER_RSP_PARAM_OUT_OF_RANGE;
        } else
        {
...

{
    ...
    if (dir == DECODER_DIR_SET)
    {
        ...
        {
            sdo_level = buffer[5];
            pin_value = (enum coines_pin_value)sdo_level;
            /*if(sdo_level == 0)
            {
...
#ifdef MCU_APP30
            coines_set_pin_config(COINES_MINI_SHUTTLE_PIN_2_3, COINES_PIN_DIRECTION_OUT, pin_value);
#else
            (void)sdo_level;
#endif
        } else /* SPI & I2C*/
 ...
 */
decoder_rsp_t decoder_0x15_multiioconfig_support(const uint8_t* buffer,uint16_t *gpio_sel_mask,uint16_t *gpio_dir_mask,uint16_t *gpio_state_mask)
{
    decoder_rsp_t rsp = DECODER_RSP_SUCCESS;
    uint16_t pin_mask;
...
decoder_rsp_t decoder_0x19_spiconfig(const uint8_t *buffer, decoder_dir_t dir)
{
    ...
    enum coines_spi_mode mode = COINES_SPI_MODE0;
    enum coines_spi_speed speed = COINES_SPI_SPEED_10_MHZ;
    ...
    if (dir == DECODER_DIR_SET)
    {
        ...
        mode = (enum coines_spi_mode)bus_mode;
        speed = (enum coines_spi_speed)bus_speed;
        /*switch(bus_mode)
        {
            case 0:
                mode = COINES_SPI_MODE0;
                break;
            case 1:
                mode = COINES_SPI_MODE1;
                break;
            case 2:
                mode = COINES_SPI_MODE2;
                break;
            case 3:
                mode = COINES_SPI_MODE3;
                break;
            default:
                break;
        ...
        if (bus_index == 1)
        {
            ...
            if (bus_mode < 4)
            {
                if (coines_config_spi_bus(COINES_SPI_BUS_0, speed, mode) != COINES_SUCCESS)
                {
                    rsp = DECODER_RSP_FAIL;
                }
            } else
